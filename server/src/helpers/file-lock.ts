...
export class FileLock {
    ...
    constructor(options?: { read?: number, write?: boolean }) {
        this._read = options?.read ?? 0
        this._write = options?.write ?? false
    }
...
export class FileLocker {
    ...
    async with_read_lock<T>(paths: string[], fn: () => Promise<T>): Promise<T> {
        return this._acquire_read_lock(paths)
            .then(release => fn().finally(release))
            .catch(e => {
                if (e.code === 'ENOENT') {
                    throw_not_found(`File not found: ${e.path}`)
                } else {
                    throw e
                }
            })
    ...
    async with_write_lock<T>(paths: string[], fn: () => Promise<T>): Promise<T> {
        return this._acquire_write_lock(paths)
            .then(release => fn().finally(release))
            .catch(e => {
                if (e.code === 'ENOENT') {
                    throw_not_found(`File not found: ${e.path}`)
    ...
    private async _acquire_read_lock(paths: string[]): Promise<() => void> {
        ...
        return () => {
            for (const p of parts) {
                if (this._locks.get(p)?.release_read()) {
    ...
    private async _acquire_write_lock(paths: string[]): Promise<() => void> {
        ...
        parts.forEach(p => this._locks.set(p, new FileLock({ write: true })))
